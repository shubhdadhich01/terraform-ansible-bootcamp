...
**Philosophy:** The bootcamp follows a single organization — **Northwind Retail** — through one continuous cloud journey. Each day represents the next phase of the same production environment: the platform team builds the landing zone, security locks down the data plane, the application team ships an internet-facing workload, security audits it, compliance forces stronger encryption, platform introduces governance, and development adds event-driven workloads. Nothing is rebuilt from scratch — every day **extends, enhances, integrates, secures, reuses, or expands** what already exists, exactly as a real platform rollout evolves. VM sizing stays on B-series/low-tier SKUs throughout; where a genuine production control (Azure Firewall, Application Gateway + WAF, Log Analytics, Private Link) adds real cost, that's called out explicitly and kept to the smallest viable SKU.

**Production practices used from Day 1 onward:** a modular repository structure (`terraform/modules`, `terraform/environments`, `ansible/roles`, `ansible/inventory`, `ansible/playbooks`, `scripts/`); an Azure Storage **remote state backend with state locking** for team collaboration; standardized resource naming (`rg-northwind-prod`, `vnet-hub-prod`, `vm-web-prod`); a consistent tagging strategy (`Environment`, `Owner`, `Project`, `Application`, `CostCenter`, `ManagedBy`); Terraform outputs used to **dynamically generate the Ansible inventory** rather than hand-editing it; and one standard daily workflow — `terraform fmt` → `validate` → `tflint` → `checkov`/`tfsec` → `plan` → `apply` → `output` → Ansible run → validation → cost review → `destroy`. Every day also includes a realistic troubleshooting exercise (routing, DNS, private endpoints, WAF, policy, managed identity, or function networking) and a brief cost validation step (estimated cost, resource review, cleanup).

---

## Day 1: Enterprise Hub-Spoke Foundation with Centralized Firewall Egress

**Scenario:** Northwind Retail's platform team is standing up the company's first Azure landing zone. Rather than a flat VNet, they build the real pattern: a hub VNet housing shared security services, and a spoke VNet for the retail web workload, with **all outbound internet traffic centrally inspected** — this is the actual starting point of nearly every enterprise Azure environment, not a simplified two-subnet demo. This foundation is what every later day extends.
...
- Azure Firewall **Policy** with rule collection groups (network rules, application rules) — the current production approach, not legacy classic firewall rules
- Azure Bastion (Basic SKU) in the hub for admin access — no jump boxes, no public IPs on any workload VM ever
- A brief architectural comparison of **hub-spoke vs. mesh vs. flat network** topologies, and why hub-spoke is the right starting point here

**Terraform:**
- Modular repo structure introduced here (`modules/network`, `modules/compute`, `environments/prod`) and used for the rest of the bootcamp
- Remote state on an **Azure Storage backend with state locking** configured from Day 1, including a short note on state migration and team collaboration workflows
- Hub/spoke VNets, peering (both directions, `allow_gateway_transit` / `use_remote_gateways`)
- Azure Firewall Policy resource with rule collection groups
- Route tables + route table associations forcing spoke egress through the firewall's private IP
- Standardized naming (`rg-northwind-prod`, `vnet-hub-prod`, `vm-web-prod`) and tagging applied to every resource
- 2 small Linux VMs (web + app tier, B1s), no public IPs

**Ansible:**
- Inventory generated dynamically from Terraform outputs rather than hand-edited
- Organized into reusable roles (`webserver`, `hardening`) rather than one large playbook — the pattern used all week
- Configure Nginx (web) and a sample backend app (app tier)
- Validate egress control from inside the VM (`curl`, `route`, `traceroute`, `nslookup` against allowed vs. blocked FQDNs, proving the firewall rule actually works)
- Harden both VMs: SSH key-only, unattended-upgrades, local firewall rules mirroring NSGs

**Troubleshooting:** Students are handed an intentionally broken UDR and must diagnose why egress traffic isn't reaching the firewall — the classic centralized-egress failure mode in real environments.
...
**Scenario:** Northwind Retail's security team now secures the data plane sitting behind Day 1's landing zone. A security review requires **zero public network access** to any data-plane service, and secrets must never be retrievable except by explicitly authorized identities — this is what "private by default" actually looks like end-to-end, not just one private endpoint.
...
**Terraform:**
- Extends the Day 1 modules; new `modules/data` module added to the same repo structure, same naming and tagging standards applied
...
**Discussion points:** Managed Identity vs. Service Principal (why the former eliminates credential management entirely), and Storage Account Keys vs. SAS tokens vs. RBAC as the three access models — with RBAC as the production-recommended default used here.

**Troubleshooting:** Students diagnose a Private Endpoint that resolves to the public IP instead of the private one — the most common real-world Private DNS misconfiguration, and a direct extension of Day 1's DNS/routing foundation.
...
**Scenario:** Northwind Retail's application team is ready to launch the retail storefront on top of the landing zone and private data plane built on Days 1–2. Flash sales create unpredictable spikes and the site is a public target for bot traffic and common web exploits. A raw Load Balancer isn't the production answer — **Application Gateway with WAF_v2** is the actual pattern for internet-facing HTTP(S) workloads that need both scaling and OWASP-grade protection.

**Choosing the right front door:** a brief comparison of Application Gateway, Azure Front Door, Load Balancer, and Traffic Manager, and why regional Layer-7 protection with WAF is the right fit for this workload versus the others. Rolling, blue-green, and canary deployment strategies are introduced conceptually as the natural next step once this ingress layer exists.
...
**Troubleshooting:** Students diagnose an unhealthy backend health probe (misconfigured `/healthz` path or NSG blocking the App Gateway subnet) — the most common real-world App Gateway deployment issue.
...
**Scenario:** With the storefront live, Northwind Retail's security team runs its first audit of the environment built across Days 1–3. Auditors require proof that **no standing administrative access exists anywhere**, every access request is time-boxed, and every network flow is captured — this is the real "prove it" bar production security teams are held to, not a checkbox.
...
**Investigating security events:** students use Azure Monitor to correlate **Activity Logs, Audit Logs, Resource Logs, Firewall Logs, and NSG Flow Logs** into a single incident timeline — the actual workflow a SOC analyst follows, not five disconnected log sources.

**Troubleshooting:** Students diagnose an unhealthy Azure Monitor Agent (stopped heartbeat, missing data in Log Analytics) — a common real-world gap between "logging configured" and "logging actually flowing."
...
**Scenario:** A major Northwind Retail customer contract now requires **customer-managed keys with documented rotation**, not just "encryption enabled" — the actual bar for regulated data, where "Microsoft manages the keys" isn't an acceptable answer. Compliance asks the platform team to raise the bar on the environment audited on Day 4.

**Key management models:** a brief comparison of Platform-Managed Keys, Customer-Managed Keys, Bring-Your-Own-Key, and Hold-Your-Own-Key, and why CMK is the right fit here versus the alternatives.
...
**Discussion points:** soft delete, purge protection, key backup/restore, and what happens on rotation failure — the operational realities behind "just enable CMK" that real key-management runbooks have to cover.
...
**Scenario:** Northwind Retail now has multiple teams deploying into the environment built across Days 1–5, and the platform team needs **org-wide guardrails enforced automatically**, plus a way to prove compliance to leadership — the actual "governance at scale" problem, addressed with an Azure Policy **initiative** (policy set), not a single standalone policy.

**Governance toolbox:** a brief comparison of Azure Policy, RBAC, Management Groups, Resource Locks, and Resource Groups — each solves a different governance problem, and this day focuses on Policy while clarifying where the others fit. Policy effects (**Audit, Deny, Modify, DeployIfNotExists**) are introduced as the mechanism that determines how strictly a rule is enforced.
...
**Troubleshooting:** Students diagnose a `terraform apply` rejected by a policy Deny effect, tracing the failure back to the specific initiative rule — the real debugging loop governance introduces into everyday deployments.
...
**Scenario:** Northwind Retail's development team needs uploaded shipment manifests to trigger validation and downstream processing automatically, integrated into the **same secured, governed, observed environment** built across the week — not a standalone serverless demo, but a new workload landing into an already-governed landing zone, which is how it actually gets built in production.

**Choosing the right compute:** a brief discussion of when to reach for Function Apps versus Azure Container Apps, App Service, or AKS — Consumption-plan Functions fit this event-driven, bursty workload; the others are the right call for different shapes of workload, and that distinction matters at architect level.
...
**Observability roles:** Application Insights captures per-execution telemetry, Log Analytics stores and queries it, and Azure Monitor alerts on it — three distinct responsibilities in the same observability pane used since Day 2.

**Troubleshooting:** Students diagnose a Function that can't reach the private Storage/Key Vault endpoints (missing VNet integration or a delegated-subnet misconfiguration) — the defining real-world gap between a tutorial Function and a production one.
...
## Capstone (Separate from the 7-Day Curriculum) — Architect Assessment: "Apex Bank" Secure Internal Portal

> This is an **assessment, not a guided lab**, delivered separately from Days 1–7. Students receive a brief and must design and justify the architecture themselves — it evaluates architectural decision-making, not the ability to follow instructions.

**Students are given:**
- Business requirements (an internal employee portal for a regulated-industry organization)
- A target architecture description (at the level of a stakeholder brief, not a resource list)
- A budget ceiling
- Security and compliance requirements (data-plane privacy, encryption, auditability, governance)

**Students must independently decide and justify:**
- Terraform module structure and remote state approach
- Firewall and network segmentation design
- Where private endpoints are required vs. optional
- Managed Identity scope and role assignments
- Which Azure Policy controls to apply and at what scope
- Ansible role structure and what gets automated vs. manually reviewed

**Evaluation focus:** the capstone is graded on the soundness of the architectural decisions — not on reproducing Days 1–7 verbatim — while implicitly drawing on every pattern the week covered: centralized egress, private-by-default data, WAF-protected ingress, JIT administration, CMK encryption, policy governance, and VNet-integrated serverless compute.

**Cost control:** Same budget discipline as the week — B-series VMs, PaaS services at minimum viable SKU, full teardown via `terraform destroy` with a documented checklist.
